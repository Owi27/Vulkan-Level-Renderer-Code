...
// lets pop a window and use Vulkan to demonstrate descriptor sets & drawcall instancing
int main()
{
	GWindow win;
	...
	{
		win.SetWindowName("Vulkan DescriptorSets Sample (+instancing)");
		...
		msgs.Create([&](const GW::GEvent& e) {
			...
			if (+e.Read(q) && q == GWindow::Events::RESIZE)
				clrAndDepth[0].color.float32[2] += 0.001f; // slow blue
		});
		win.Register(msgs);
#ifndef NDEBUG
